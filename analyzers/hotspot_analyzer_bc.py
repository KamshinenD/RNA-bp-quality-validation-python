"""Hotspot analyzer with pre-computed lookups for speed."""
...
class HotspotAnalyzer:
    ...
    def find_hotspots(self, basepair_data: list, hbond_data: pd.DataFrame) -> List[Hotspot]:
        ...
        # ===== Pre-compute all H-bond lookups ONCE =====
    ...
    def _precompute_hbond_data(self):
        ...
        This is the KEY optimization that speeds up everything for me.
        """
    ...
    def _is_problematic_base_pair(self, bp: dict) -> Tuple[bool, str]:
        ...
        1. Check if geometry is bad (SEVERE, MODERATE, or MINOR)
        2. If geometry is bad, then check if there are ANY H-bond issues
        ...
        # Step 1: Is geometry bad at all?
        if geometry_severity == 'NONE':
            return False, 'good_geometry'
        
        # Step 2: Geometry IS bad, now check for H-bond issues
        has_hbond_issues = self._bp_has_hbond_issues_fast(bp)  # ← FAST VERSION
        
        if not has_hbond_issues:
            return False, f'{geometry_severity.lower()}_geometry_but_no_hbond_issues'
        
        # Both conditions met: bad geometry + H-bond issues
        if geometry_severity == 'SEVERE':
            return True, 'severe_geometry_plus_hbond'
        elif geometry_severity == 'MODERATE':
            return True, 'moderate_geometry_plus_hbond'
        elif geometry_severity == 'MINOR':
            return True, 'minor_geometry_plus_hbond'
        
        return False, 'acceptable'
    
    def _bp_has_hbond_issues_fast(self, bp: dict) -> bool:
        """
        OPTIMIZED: Check if base pair has ANY H-bond issues using pre-computed data.
        
        H-bond issues include:
        - Zero H-bond score (complete absence)
        - Incorrect H-bond count
        - Bad geometry (distance, angles, dihedral, quality)
        
        This is O(1) instead of O(n) where n = number of H-bonds.
        
        Returns: True if ANY H-bond issue exists
        """
        hbond_score = bp.get('hbond_score', 0)
        pair_key = tuple(sorted([bp['res_1'], bp['res_2']]))
        
        # Issue 1: Zero H-bond score
        if hbond_score == 0.0:
            return True
        
        # Issue 2: H-bond geometry problems (distance, angles, dihedral, quality)
        # These were pre-computed in _precompute_hbond_data()
        if pair_key in self._pair_has_issues:
            return True
        
        # Issue 3: Incorrect H-bond count
        actual_count = self._pair_hbond_count.get(pair_key, 0)
        
        is_incorrect, _ = HBondScoring.check_hbond_count(
            bp.get('bp_type', 'unknown'), 
            actual_count, 
            bp.get('lw', ''), 
            self.config
        )
    ...
    def _create_hotspot_from_residues(self, chain: str, residues: Set[int]) -> Hotspot:
        ...
        MODIFIED: Apply smart boundary detection based on consecutive good pairs.
        - Trims boundaries where 3+ consecutive good pairs exist
        - Requires minimum 3 problematic base pairs
        - Allows 1-2 good pairs to bridge problematic regions
        """
        residue_list = sorted(residues)
        
        # NEW: Trim boundaries where 3+ consecutive good pairs exist
        trimmed_residues = self._trim_hotspot_boundaries(chain, residue_list)
        
        if len(trimmed_residues) < 3:
            return None
        
        start_res = min(trimmed_residues)
        end_res = max(trimmed_residues)
        
        # Check for huge gaps
        gaps = [trimmed_residues[i+1] - trimmed_residues[i] for i in range(len(trimmed_residues)-1)]
        if gaps and max(gaps) > 50:
            return None
        
        span = end_res - start_res + 1
        density = len(trimmed_residues) / span
        ...
        # VALIDATION: Require minimum 3 problematic base pairs (CHANGED from 1)
        if num_problematic < 3:
            return None
        
        # VALIDATION: Check fraction threshold (KEPT but may be less relevant now)
        ...
        issue_density = self.calculate_issue_density(set(trimmed_residues), all_region_bps, region_hbs)
        ...
        # Store ALL base pairs data
        all_base_pairs_data = [
            {'res_1': bp['res_1'], 'res_2': bp['res_2']}
            for bp in all_region_bps
        ]
        
        print(f"DEBUG: Creating hotspot {chain}:{start_res}-{end_res} with {len(all_base_pairs_data)} total base pairs, {num_problematic} problematic ({fraction_bad:.1%})")
        
        return Hotspot(
            chain=chain,
            start_res=start_res,
            end_res=end_res,
            residue_count=len(trimmed_residues),
            ...
            details={
                ...
                'hbonds': hb_results,
                'residues': trimmed_residues,  # ← Use trimmed residues
                'detailed_issues': detailed_issues,
                'all_base_pairs': all_base_pairs_data,
                'problematic_base_pairs_count': num_problematic,
                'fraction_problematic': round(fraction_bad, 2),
                'problem_type_breakdown': dict(problem_types),
                'issue_statistics': {
                    'total_bases': len(trimmed_residues),  # ← Use trimmed count
    ...
    def _trim_hotspot_boundaries(self, chain: str, residue_list: List[int]) -> List[int]:
        """
        Trim hotspot boundaries where 3+ consecutive good base pairs exist.
        
        This ensures hotspots stop at clear quality boundaries, not arbitrary cutoffs.
        """
        if len(residue_list) < 3:
            return residue_list
        
        # Classify each residue as problematic or good
        residue_quality = {}
        ...
        for res in residue_list:
            ...
            for bp in self._basepair_cache:
                chain1, r1 = self._parse_residue(bp['res_1'])
                chain2, r2 = self._parse_residue(bp['res_2'])
                
                # Check if this residue is involved
                if (chain == chain1 and res == r1) or (chain == chain2 and res == r2):
                    is_bad, _ = self._is_problematic_base_pair(bp)
                    
                    if is_bad:
                        has_problematic_pair = True
                        break
            
            residue_quality[res] = has_problematic_pair
        
        # Trim from start: skip initial consecutive good residues (3+)
        start_idx = 0
        consecutive_good = 0
        
        for i, res in enumerate(residue_list):
            if not residue_quality.get(res, False):
                # This is a good residue
                consecutive_good += 1
            else:
                # Found a problematic residue
                # If we had 3+ consecutive good before this, skip them
                if consecutive_good >= 3:
                    start_idx = i  # Start at this problematic residue
                else:
                    start_idx = i - consecutive_good  # Keep the good ones that bridge
                break
        else:
            # All residues were good - return empty
            if consecutive_good >= 3:
                return []
        
        # Trim from end: skip trailing consecutive good residues (3+)
        end_idx = len(residue_list)
        consecutive_good = 0
        
        for i in range(len(residue_list) - 1, start_idx - 1, -1):
            res = residue_list[i]
            if not residue_quality.get(res, False):
                # This is a good residue
                consecutive_good += 1
                ...
                # Found a problematic residue
                # If we had 3+ consecutive good after this, skip them
                if consecutive_good >= 3:
                    end_idx = i + 1  # End after this problematic residue
                else:
                    end_idx = i + consecutive_good + 1  # Keep the good ones that bridge
                break
        else:
            # All remaining residues were good
            if consecutive_good >= 3:
                end_idx = start_idx  # Return empty
        
        # Return trimmed list
        if start_idx >= end_idx:
            return []
        
        return residue_list[start_idx:end_idx]